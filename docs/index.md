...
For Normalized Cut, we used scikit-learn's implementation for the method itself. For their implementation, we had to generate a Region Adjacency Graph(RAG) for which we used the skimage implementation. This meant that instead of considering each pixel as one region (like we have done for other methods), we first used k-means to segment the picture into regions, created the RAG from these regions and then applied normalized cut. This is because, considering each pixel as a region would significantly increase the time that the method takes to run for each picture. We decided to use k=400 segments for the RGB and HSV feature spaces and k=1000 segments for the ones with position in the interest of time.
An obstacle we faced in this implementation was the incompatibility of the skimage RAG generating code with our 5-channel feature spaces with position. For this, we had to change their implementation just a little bit so it would accept matrices the way we were generating them.
...
For the Normalized Cut implementation, on each picture, we first ran k-means with a large number of segments, and then used the resulting regions to make the Region Adjacenecy Graph required for a graph-based method like Normalized Cut. Here is the [output](https://cs-6476-project.herokuapp.com/?q=140006) for a sample image:
